...
//PreparedStatement ResultSet 메서드는 위에서 불러옴.
				...
				%>
						<tr>
							...
							String gender1 = rs.getString("gender");
							 // out.print(gender1); 확인용
							 // gemder1 == "M" 으로는 안되더라구요.
							if (gender1.equals("M"))
							{%>
								<span style=color:pink> 남</span>
							<%
							} else {
							%>
								<span style=color:hotpink> 여</span>
							<%
							}
								%></td>
      ...
      %>
            <a href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=<%=currentPage-1%>">이전</a>
      <%      
         }
      %>
      ...
      <%
         ...
         //보드 케이블의 총 행수 구하는 쿼리.
      %>
            <a href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=<%=currentPage+1%>">다음
            </a>
</div>
</body>
</html>