...
//콤마뒤 띄어쓰기!
			PreparedStatement stmt = conn.prepareStatement("SELECT emp_no, title, from_date, to_date from titles limit ?,?");
...
//PreparedStatement ResultSet 메서드는 위에서 불러옴.
      ...
      %>
            <a href="<%=request.getContextPath()%>/titles/titlesList.jsp?currentPage=<%=currentPage-1%>">이전</a>
      <%      
         }
      %>
      ...
      <%
         ...
         //보드 케이블의 총 행수 구하는 쿼리.
      %>
            <a href="<%=request.getContextPath()%>/titles/titlesList.jsp?currentPage=<%=currentPage+1%>">다음
            </a>
