...
// 강사님께서 안 좋은 방식이라 하심..
		...
		int beginRow = (currentPage-1)*10;
         //시작은 0
			...
			String sql ="SELECT emp_no, dept_no, from_date, to_date FROM dept_manager limit";
...
//PreparedStatement ResultSet 메서드는 위에서 불러옴.
      ...
      %>
            <a href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=<%=currentPage-1%>">이전</a>
      <%      
         }
      %>
      ...
      <%
         ...
         //보드 케이블의 총 행수 구하는 쿼리.
      %>
            <a href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=<%=currentPage+1%>">다음
            </a>
</div>
</body>
</html>